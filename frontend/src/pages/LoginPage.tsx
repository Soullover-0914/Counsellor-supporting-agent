...
export function LoginPage() {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      navigate(
        next.mustChangePassword ? '/change-password' : ROLE_HOME[next.role],
        { replace: true },
      )
    } catch (error) {
    ...
    <div className="auth-shell">
      ...
      >
        <div className="login-brand">
          ...
          <div>
            <h1>Agent 66</h1>
            <p className="login-subtitle">Counselling Support System</p>
          </div>
        </div>

        <p className="login-message">
          Connecting students with authorised human support while protecting privacy
          and maintaining appropriate institutional safeguards.
        </p>

        <Alert tone="info" icon={<Shield size={18} aria-hidden />}>
          This system routes support requests to authorised personnel. It does not
          provide counselling, diagnosis, or emergency response by itself.
        </Alert>
